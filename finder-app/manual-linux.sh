...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    #make -j8 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} vmlinux
    make -j8 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} dtbs
fi

# Copy kernel Image to OUTDIR for QEMU/autograder compatibility
...
cp -a ${SYSROOT}/lib/ld-linux-aarch64*.so* ${OUTDIR}/rootfs/lib || true
...
# Ensure username.txt exists BEFORE copying conf/
if [ ! -f "${FINDER_APP_DIR}/conf/username.txt" ]; then
    echo "felix" > ${FINDER_APP_DIR}/conf/username.txt
    echo "Created conf/username.txt with default content"
fi
